...
- **Activities** — Strava sync + Garmin / Fitbit / Google Health imports, GPS map view, side-by-side compare
...
- **Workout / Strength** — Fitbod-style generated session per day, filtered to your actual gear (dumbbell pairs, wrist-weight micro-loaders, bench config, etc.); per-set logging with rest timer; recovery-aware deload; deterministic seed so today's plan is stable until you tap regenerate; full set/rep/RPE history. Generator considers your **goal** (strength / hypertrophy / general — drives rep ranges, rest periods, weight-jump size per Schoenfeld 2010/2017), **age** (+15/30/45s rest in 40s/50s/60+, set trim on isolation/secondary at 50+/60+), and **bodyweight** (rep targets on BW exercises scale inverse to user weight). **Rotation pressure** down-ranks exercises used in the last 4 weeks. **Frequency advisory** suggests changing days/week when your actual cadence diverges from the declared setting. **Weekly muscle-volume audit** scores every primary muscle vs research-backed MEV/MAV ranges. **Equipment editor** drives catalog filtering. **Catalog view** lets you favorite / avoid / disable individual exercises. **Coach card** consolidates four AI-backed advisors: deload trigger (multi-signal recovery judgment), pre-workout focus cue, variety nudge (0-2 AI swap suggestions), and Why-this-workout rationale.
...
- **Post-workout review** — structured 4-field card generated on demand from a completed strength session, comparing against a trailing 4-week tonnage / RPE baseline
- **Pre-workout focus cue** — short coaching tip referencing today's specific lifts + 6-week per-exercise history
- **Multi-signal deload trigger** — judgment from 28d of vitals trends + 14d strength performance, severity ∈ {none, light, moderate, rest} with concrete evidence and recommendation
- **Variety nudge** — proposes 0-2 surgical exercise swaps with reasoning; Accept auto-dismisses so the AI doesn't re-query the modified plan
