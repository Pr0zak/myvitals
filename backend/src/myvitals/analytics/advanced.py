...
async def readiness_breakdown(
    ...
    if sleep_score is not None:
        ...
        drivers.append({
            # SA-N3: this was captioned "Sleep quality". `sleep_score`
            # (analytics/sleep.py) is 60% duration-distance-from-8h and a
            # deep+REM term that saturates at 100 on ~90% of this user's
            # real nights (measured against a fixed 0.30 anchor), so the
            # value is overwhelmingly a duration signal — corr with
            # |hours-8| is -0.89 in production. Every other surface that
            # reads this same column (Calendar, Compare, Trends) already
            # calls it "Sleep score"; this was the one caption claiming
            # more than the number measures. See docs/sa-findings.json
            # SA-N3 for the full measurement and why the computation
            # itself was left alone (stored column, no backfill, feeds
            # Compare/Insights/5 AI payloads).
...
async def daily_training_stress(
    ...
    """Training load for one day — activities plus completed strength
    sessions.

    Delegates to `training_load_by_day` for a single-day window rather than
    keeping a second copy of the same activity + strength arithmetic. The
    standalone version this replaced early-returned `None` the moment the
    `activities` query came back empty — three lines above where the
    strength contribution was folded in — so a day with a completed lift and
    no Strava ride read as zero training load: exactly the failure counting
    strength sessions was supposed to fix. Delegating retires that second
    copy outright instead of patching it, so the per-day and weekly-window
    views can never again disagree about how a day's load is derived.
    """
    by_day = await training_load_by_day(db, target, target)
    return by_day.get(target)


#: How far back `update_training_load` will look for a usable prior
#: (ctl, atl) pair to seed from. Comfortably past the 42d CTL horizon —
#: past this, there is no honest sense in which a row is "yesterday".
TRAINING_LOAD_SEED_LOOKBACK_D = 90
...
async def update_training_load(
    ...
    Seeds from the most recently STORED (ctl, atl) pair rather than only
    ever looking at yesterday's row, and decays the EWMA forward across any
    gap (each skipped day contributes 0 TSS — nothing was computed for it,
    so there is nothing but decay to apply) instead of falling back to a
    fabricated 0.0 the moment yesterday's own row is missing or null. That
    fallback used to reset the whole chain: because every later day seeds
    off the one immediately before it, a single missing/null "yesterday"
    pinned CTL/ATL/TSB at 0.00 from that day forward — not decay, which
    cannot reach 0.0 from a real reading in one step.

    Returns (None, None, None) — refuse rather than assert a zero, per this
    project's null-is-not-zero rule — only when nothing usable exists
    anywhere in the lookback window: a genuine first day of tracking, where
    there is no "yesterday" to have gotten wrong.
    """
    prev = (await db.execute(
        select(
            models.DailySummary.date,
            models.DailySummary.ctl,
            models.DailySummary.atl,
        )
        .where(models.DailySummary.date < target)
        .where(models.DailySummary.date >= target - timedelta(days=TRAINING_LOAD_SEED_LOOKBACK_D))
        .where(models.DailySummary.ctl.is_not(None))
        .where(models.DailySummary.atl.is_not(None))
        .order_by(models.DailySummary.date.desc())
        .limit(1)
    )).first()
    if prev is None:
        return None, None, None
    prev_date, prev_ctl, prev_atl = prev
    prev_ctl = float(prev_ctl)
    prev_atl = float(prev_atl)
    # Gap days with no stored load: nothing computed for them, so decay
    # them forward at 0 TSS before folding in today's real contribution.
    for _ in range((target - prev_date).days - 1):
        prev_ctl += (0.0 - prev_ctl) / 42.0
        prev_atl += (0.0 - prev_atl) / 7.0
    tss = float(tss_today or 0)
    ctl = prev_ctl + (tss - prev_ctl) / 42.0
    atl = prev_atl + (tss - prev_atl) / 7.0
    tsb = prev_ctl - prev_atl  # yesterday's (decayed) form
...
async def training_load_by_day(
    ...
    Also the delegate `daily_training_stress` calls for a single-day window
    (`since == until == target`) — one implementation, so a per-day figure
    and the day's contribution to a weekly window can never disagree.
    """
    tz = _local_tz()
