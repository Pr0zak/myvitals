...
def _prep_plan_system(tone: str) -> str:
    return (
        ...
        "6 meals. Count before you answer: add up every `portions` value "
        "across all meals that reference a component, and make that "
        "component's `portions` equal to the sum.\n\n"
        "## Size the batches to the budget you are given\n"
        "`per_meal_budget.per_slot` gives a kcal target for each slot. "
        "Each meal should land near it — within roughly 15%. Batches "
        "that are too small are the common failure: a 700 kcal dinner is "
        "not a chicken breast and 50 g of grain. Work backwards from the "
        "budget to the quantity, remembering that quantities are RAW "
        "weights of what you buy, so 1 kg of chicken across 6 portions "
        "is 167 g raw each.\n\n"
        ...
        "## One ingredient per component\n"
        "A component is ONE thing the server can look up and cost. If "
        "you roast broccoli and peppers on the same tray, that is two "
        "components, not one called 'roasted broccoli and peppers' — "
        "only the first ingredient gets costed and the peppers silently "
        "vanish from the nutrition totals. Combining them in a MEAL is "
        "exactly right; combining them in a component is not.\n\n"
